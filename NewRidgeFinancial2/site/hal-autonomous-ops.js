...
const HalAutonomousOps = (function () {
  ...
  async function runTick(ctxProvider) {
    ...
    if (hasRuntime() && now - lastHealAt >= HEAL_MIN_MS) {
      ...
      try {
        ...
        if (PS && PS.runAutonomousHealLoop) {
    ...
    if (now - lastPlacementAt >= PLACEMENT_MIN_MS && typeof HalProactive !== "undefined" && HalProactive.runCycle) {
      lastPlacementAt = now;
      try {
  ...
  function start(ctxProvider) {
    ...
    running = true;
    timer = setInterval(() => {
      runTick(ctxProvider).catch(() => {});
    }, TICK_MS);
    setTimeout(() => runTick(ctxProvider).catch(() => {}), 8000);
  }

<#
.SYNOPSIS
  Keep only hal-local:30b-a3b (+ base qwen3:30b-a3b) on disk and pin it on the R9700 GPU.

.DESCRIPTION
  Evicts non-MoE residents, removes other Ollama tags, re-applies single-model
  GPU env, and pins hal-local:30b-a3b with keep_alive=-1.
#>
[CmdletBinding()]
param(
    [switch]$SkipDelete,
...
$keep = @{
    "hal-local:30b-a3b" = $true
    "qwen3:30b-a3b-instruct-2507-q4_K_M" = $true
}

Write-Host ""
Write-Host "=== Enforce HAL MoE-only (GPU + disk) ===" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "Applying single-MoE GPU pin..." -ForegroundColor Yellow
& $applyScript

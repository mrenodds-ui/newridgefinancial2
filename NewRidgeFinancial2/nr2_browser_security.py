...
def origin_allowed_for_mutation() -> bool:
    ...
    if origin is None or str(origin).strip().lower() in ("null", ""):
        return False
    ...
    if origin_norm in allowed:
        return True
    return origin_norm.startswith("http://127.0.0.1") or origin_norm.startswith("http://localhost") or origin_norm.startswith(
        "https://127.0.0.1"
...
def abort_browser_auth(reason: str, detail: str) -> None:
    bottle.response.content_type = "application/json"
    bottle.abort(403, json.dumps({"ok": False, "error": "browser_mutation_forbidden", "reason": reason, "detail": detail}))

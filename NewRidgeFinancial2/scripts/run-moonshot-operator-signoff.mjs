...
function writeReport() {
  ...
  const moonshotVerdict =
    checklistFails.length === 0 && checklist.filter((r) => r.status === "PASS").length >= 8
      ? "APPROVE hal-10092 — Moonshot data lane: payments, operatory contract, procedures export, hub sign-off (automated sign-off; operator name still required)"
...
async function main() {
  ...
  record(
    ...
    moonshotExtentSrc.includes("hal-10092") && moonshotExtentSrc.includes("Practical ceiling") ? "PASS" : "FAIL",

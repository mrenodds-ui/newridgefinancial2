"""Moonshot AI — What's next after OM clinical notes + treatment estimates.

Operator: continue
Just shipped: 4039bf4 Add clinical notes and treatment estimates to OM mini-dossier.
"""
...
- 4039bf4 — OM mini-dossier clinical notes + treatment estimates
  (om_patient_dossier.get_patient_dossier_mini: clinicalNotes[], treatmentEstimates[];
   optical OM panel sections; empty ≠ $0; SoftDent READ-ONLY)
- 2cd6959 — HAL patient-context bind + auto-summarize
- 873b4c6 / 16197d2 / 2310d0d — Mon–Thu list, provider filter, claims, Ask HAL

OPEN RUNNER-UPS from prior consults:
1) Desk smoke extension: Mon–Thu → dossier → HAL → Force Close / beam proof
2) HAL “this patient” chat shortcut using bound session context (no re-ask id)
3) SoftDent/Trellis tomorrow eligibility worklist (nightly/batch helpers exist)
4) Classic Apex OM weekly widget restore (2B) — optional only

CONSTRAINTS:
- SoftDent READ-ONLY; empty ≠ $0; PHI hash/initials on OM board
- Do NOT redo Mon–Thu list, mini dossier base, claims, clinical notes, estimates,
  Ask HAL bind, patient-context API, or auto-summarize
- Use REAL NR2 paths only (NewRidgeFinancial2/site/nr2-optical-*, om_patient_dossier.py,
  nr2_http_server.py, desk_smoke.py, patient_dossier.py) — never invent src/integrations/

YOUR JOB: Pick THE single best NEXT package.

CANDIDATES (pick ONE):
1) HAL “this patient” policy shortcut — resolve bound patientContext when query says
   this patient / about this patient without repeating SoftDent id
2) Desk smoke extension covering OM Mon–Thu + patient-context + summarize + Force Close/beam
3) SoftDent/Trellis tomorrow-insurance worklist hardening / nightly verify
4) Classic Apex 2B thin weekly schedule widget
...
def live_audit() -> dict:
    ...
    try:
        ...
        build = {"error": str(exc)}
    om = (NR2 / "om_patient_dossier.py").read_text(encoding="utf-8", errors="replace")
    store = (NR2 / "hal_session_store.py").read_text(encoding="utf-8", errors="replace")
    return {
        "repoRoot": str(REPO),
        "build": build,
        "commitHint": "4039bf4 clinical notes + treatment estimates on OM mini-dossier",
        "markers": {
            "clinicalNotesInMini": "clinicalNotes" in om and "treatmentEstimates" in om,
            "setPatientContext": "def set_patient_context" in store,
            "thisPatientShortcut": False,
        },
        "live": {
...
def main() -> int:
    ...
    if "openrouter.ai" in base_url:
        ...
        headers["X-Title"] = (
            os.getenv("OPENROUTER_X_TITLE") or "NR2 Whats Next After OM Clinical Mini"
        )
    ...
    raw_path = OUT / f"moonshot_whats_next_after_om_clinical_{stamp}.json"
    md_path = DOCS / f"MOONSHOT_WHATS_NEXT_AFTER_OM_CLINICAL_MINI_{DATE}.md"
    raw_path.write_text(json.dumps(raw, indent=2)[:500000], encoding="utf-8")

    header = (
        f"# Moonshot AI — What's Next After OM Clinical Mini-Dossier (CONSULT ONLY)\n\n"
        f"**Date:** {DATE}\n"
        f"**Model:** `{model}`\n"
        f"**Key:** {key_name}\n"
        f"**Script:** `scripts/run_moonshot_whats_next_after_om_clinical_mini_consult.py`\n"
        f"**Shipped:** `4039bf4`\n"

...
QUANT_TYPE="${2:-Q4_K_M}"
ROOT="$(repo_root)"
OUT_DIR="${MODEL_OUTPUT_DIR:-$ROOT/.local_models/frontend}"
FALLBACK_QUANTS=("Q4_K_S" "Q3_K_M")

if [[ -z "$SOURCE_PATH" ]]; then
  echo "Usage: $0 /path/to/source-24b-model [Q4_K_M]" >&2
  exit 1
fi
...
OUT_GGUF="$OUT_DIR/frontend-24b.${QUANT_TYPE}.gguf"
...
print_vram_guidance "frontend" "$QUANT_TYPE"
...
if ! "$QUANT_BIN" "$SOURCE_GGUF" "$OUT_GGUF" "$QUANT_TYPE"; then
  for fallback in "${FALLBACK_QUANTS[@]}"; do
    ...
    OUT_GGUF="$OUT_DIR/frontend-24b.${fallback}.gguf"
...
echo "  AI_FRONTEND_MODEL_PATH=$OUT_GGUF"
echo "  AI_FRONTEND_QUANT=$QUANT_TYPE"

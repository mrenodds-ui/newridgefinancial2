"""Moonshot AI — What next after REC-009 voice context carry (CONSULT ONLY)."""
...
- Expert SE REC-008 batch claim narratives (consent + batch generate + print packet)
- Expert SE REC-009 voice context carry (focused claim → session POST + Narratives
  voice [ClaimRef] resolve; 5‑min TTL; tests passed)
- Live: readiness fresh; only optional QB payroll+AP missing; critical completeness 100%

Expert SE NICE items REC-008 and REC-009 are NOW CLOSED.
Open SHOULD leftovers from Expert SE (if still unfinished locally): REC-005 ERA parser depth,
REC-007 cache warming, etc. — verify against applied docs; do not redo shipped work.
Optional: QB payroll/AP file exports into inbox; gitignore site/index.pre-apex.html.

REAL PATHS only (NewRidgeFinancial2/apex_*.py, site/apex-*.js, hal_learning, etc.).
...
def main() -> int:
    ...
    user = (
        ...
        "What should we do NEXT on NR2 Apex locally after REC-009 shipped? CONSULT ONLY.\n"
    ...
    if "openrouter" in base_url.lower():
        ...
        headers["X-Title"] = "NR2 Whats Next After REC-009"
    ...
    try:
        ...
        status = "error"
    header = (
        f"# Moonshot AI — What's Next After REC-009 (CONSULT ONLY)\n\n"
        f"**Date:** {DATE}  \n**Model:** {model}  \n**Key:** {key_name}  \n"
        f"**Status:** {status}  \n**Build:** hal-10561 + hal-local:32b  \n"
        f"**Script:** `scripts/run_moonshot_whats_next_after_rec009_consult.py`  \n"
        f"**Apply:** DO NOT APPLY until operator approves.\n\n"
        f"## Operator request (verbatim)\n\n> {OPERATOR_REQUEST_VERBATIM}\n\n"
        f"**Constraint:** avoid GitHub / PR for now.\n\n---\n\n"
    )
    full = header + (content or "(empty)")
    out = OUT / f"MOONSHOT_WHATS_NEXT_AFTER_REC009_{DATE}.md"
    doc = DOCS / f"MOONSHOT_WHATS_NEXT_AFTER_REC009_{DATE}.md"
